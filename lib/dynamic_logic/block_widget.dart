...
final blockWidgetFactory = <Type, Widget Function(Block)>{
  NextButtonBlock: (block) => NextButton(block: block as NextButtonBlock),
  TimerBlock: (block) {
...
final List<Step> werewolfGame = [
  ...
  Step(
    "gameStarted",
    TagCondition.parse("game.startedAlready = 0"),
    ChangeTagBlock(tags: const [Tag("game.startedAlready")]),
  ),
  const Step(
  ...
  Step(
    ...
    const NextButtonBlock(endsGame: true, text: "Werewolfs have won"),
  ...
  Step(
    ...
    const NextButtonBlock(endsGame: true, text: "Villagers have won"),
